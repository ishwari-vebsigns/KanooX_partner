...
class AdminController extends Controller
{
     ...
     //        $role=$user->role_id;
		if ($user->role_id!=1 && $user->role_id!=2) {
		  ...
		  //  $thirty_days=date('Y-m-d', strtotime('- 30 days'));
			...
			return view('admin.dashboard')->with('user_count',$user_count)->with('refer_earn_count',$refer_earn_count);
	...
	public function getusernew(Request $request){
		...
		// dd($loan_signin_id, $user_loan);
			
		// }	
		return view('admin.user-apply')->with('user_loan', $user_loan);
	}
	public function postusernew(Request $request){
		...
		$request->validate([
			'user_name' => 'required',    
			'fname' => 'required',    
			'phone' => 'unique:users,contact_number|digits:10',    
			...
			'city' => 'required',    
			'state' => 'required',    
			'pan_card' => 'unique:loans,pan_card',    	
			'dob' => 'required|date',    
		 ]);
		...
		return redirect('admin/apply-user');
	}
