...
class SupportController extends Controller
{
    public function getedithelpandSupport(Request $request){
        if(!$this->checkPermission(Config::get('permissions.SUPPORT_DETAILS'))){
			return view('admin.unauthorized');
		}
        $id = $request->id;
        $support = Support::where('support_id', $id)->with('user')->first();
        $comments = Chat::where('support_id', $id)->with('username')->get();
        // dd(Chat::get());
    ...
    public function getagentcomplaint(){
        if(!$this->checkPermission(Config::get('permissions.SUPPORT_ADD'))){
			return view('admin.unauthorized');
		}
        // dd(Chat::get());die;
     ...
     public function getdeletesupportall(Request $request){
        $id = $request->id;
        // dd($id);
        $supportdelete = Support::find($id);
        $supportdelete->delete();
        return redirect('admin/help&support/all');
    }
    public function postagentcomplaint(Request $request){
        ...
        try {
            ...
            // dd($support);die;
            $comment = new Chat();
            $comment->support_id = $support->getKey(); // Use the "getKey()" method to retrieve the primary key value
            ...
            // Handle the query exception and log or display an error message
    ...
    public function gethelpandSupport(){
        if(!$this->checkPermission(Config::get('permissions.SUPPORT_ALL'))){
			return view('admin.unauthorized');
		}
