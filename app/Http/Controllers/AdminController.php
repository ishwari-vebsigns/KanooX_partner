...
use App\Models\Bankresponse;
use App\Models\Custdocument;
use Hash;
...
class AdminController extends Controller
{
	...
	public function getallcustomersalldata(Request $request)
	{
		...
		$id = Auth::user()->id;
		if (Auth::user()->role_id != 2 && Auth::user()->role_id != 3) {
			$customers = Loansignin::whereDate('created_at', '>=', $startDate)
				->with('agent')
				->with('subservice')
				->whereDate('created_at', '<=', $endDate)
				->get();
		...
		if (Auth::user()->role_id == 2) {
			// dd()
			// $customers = Loansignin::where('agent_id',Auth::user()->id)->with('agent')->whereDate('created_at', '>=', $startDate)->whereDate('created_at', '<=', $endDate)->get();
			$customers = Loansignin::where('agent_id', Auth::user()->id)->with('agent')
				->with('subservice')
	...
	public function gettraining()
	{
		...
		$trainings = Training::where('status_id', 1)->get();
	...
	public function posteditsubservice(Request $request)
	{
		...
		if (isset($request['save'])) {
			$request->validate([
				'sub_service' => [
					'required',
					Rule::unique('services', 'service_name')->ignore($id, 'service_id'),
				],
				'sub_service_url' => [
					'required',
					Rule::unique('services', 'service_url')->ignore($id, 'service_id'),
				],
				'description' => 'required',
				'sub_service_image' => 'image|mimes:jpeg,png,jpg,gif,webp|max:5000',
			]);
			// dd($request->all());
			...
			if ($request->hasFile('sub_service_image_2')) {
				$path2 = $request->sub_service_image_2->store('sub-service-images');
				$serviceshierarchy->sub_service_image_2 = $path2;
			}
	...
	{
		$services = Service::where('is_main_service', 0)->with('service')->get();
	...
	public function getpostServices(Request $request)
	{
		...
		if (isset($request['save'])) {
			$request->validate([
				'service_name' => [Rule::unique('services', 'service_name')->ignore($id, 'service_id')],
				'service_url' => [Rule::unique('services', 'service_url')->ignore($id, 'service_id')],
				'service_image' => 'nullable|image|mimes:jpeg,png,jpg,gif,webp|max:5000',
			]);
			...
			if ($request->hasFile('service_image')) {
				$path = $request->service_image->store('service-images');
	...
	public function postaddServices(Request $request)
	{
		...
		$service = new Service();
		$service->service_name = $request->service_name;
		$service->service_url = str::slug($request->service_url);
		$service->is_main_service = 1;
	...
	public function getserviceType(Request $request)
	{
		...
		if (Auth::user() != null) {
			...
			$services = ServicesHierarchy::where('parent_service_id', $service_url)->where('status_id', 1)->get();
	...
	public function getServices(Request $request)
	{
		...
		return view('admin.services')->with('code', $code)->with('services', $services);
	}
	...
	public function getDashboard(Request $request)
	{
		...
		if ($user->role_id != 1 && $user->role_id != 2 && $user->role_id != 3) {
			...
			$loan_leads_count = DB::table('basic_infos')->count();
			...
			$completed_loan_count = $loan_status_counts['completed'];
			$conversion_rate = $total_loan_leads > 0 ? round(($completed_loan_count / $total_loan_leads) * 100) : 0;
			// ↓↓↓ YAHAN NAYA CODE ADD KARNA HAI ↓↓↓
...
// 	}
	...
	{
		$agent_code = $request->agent_code;
		$agentqrc = User::where('agent_access_code', $agent_code)
			->with('agent_qr')
			->first();
	...
	public function getusernew(Request $request)
	{
		...
		$sub_service_id = $request->session()->get('sub_service_id');
		$user_selected_bank = $request->session()->get('user_selected_bank');
		// dd($user_selected_bank);
		$main_service = ServicesHierarchy::where('child_service_id', $sub_service_id)->first();
		// dd($user_selected_bank);
		if ($loan_signin_id == null) {
			session()->forget('sub_service_id');
			// dd($main_service->parent_service_id);die;
			return redirect('admin/services/' . $main_service->parent_service_id);
		}
		// dd($loan_signin_id);	 
		...
		if ($user_selected_bank == 53) {
			return view('admin.user-apply-lendingkart')->with('user_loan', $user_loan)->with('code', $code);
		} elseif ($user_selected_bank == 56) {
			return view('admin.user-apply-cashe')->with('user_loan', $user_loan)->with('code', $code);
		} else {
	...
	// 	$loan->purpose_of_loan = $sub_service_id;
	...
	// 	$loan->save();
	// 	// dd($loan);die;
	// 	$nameParts = explode(" ", $loan->full_name);
	// 			// dd($nameParts);die;
	...
	// 			if($user_selected_bank== 56){
	// 				$url = 'https://test-partners.cashe.co.in/partner/create_customer';


	// 				$payload = [
	// 					"partner_name" =>'BharatNidhi_Partner',
	// 					"loan_amount" => $loan->loan_amount,
	// 					"Personal Information" => [
	// 						"First Name" => $firstName,
	// 						"Last Name" => $lastName,
	// 						"Gender" => $loan->gender,
	// 						"Address Line 1" => $loan->residence_address,
	// 						"Pincode" => $loan->zip_code,
	// 						"City" => $loan->city,
	// 						"State" => $loan->state,
	// 						"PAN" => $loan->pan_card,
	// 					],
	...
	// 						"Monthly Income" => $loan->month_one_net_salary,
	// 						"Employment Type" => $loan->profession_type,
	// 						"Salary ReceivedTypeId" => $loan->salary_type,
	// 					],
	// 					"Contact Information" => [
	// 						"Mobile" => $loan->mobile,
	// 						"Email Id" => $loan->email
	...
	// 						"name" => $loan->full_name,
	// 						"gender" => $loan->gender,
	// 						"compressed-address" => $loan->residence_address
	// 					]
	// 				];
	...
	// 	$request->session()->put('cancel',"Customer Loan Cancelled Successfully!!");
	// 	session()->forget('loan_signin_id');
	// 	if(Auth::user()!=null){
	// 		return redirect('admin/services/form/apply-user');
	// 	}else{
	...
	public function postusernew(Request $request)
	{
		...
		if ($user_selected_bank == 53) {
			if (isset($request['save'])) {
				$request->validate([
					'user_name' => ['required', 'regex:/^[\p{L} .\'-]+$/u'],
					'phone' => 'required|digits:10',
					'pincode' => 'required|digits:6',
					'business_age' => ['required', 'numeric', 'gt:3'],
					'business_revenue' => ['required', 'numeric', 'gt:300000'],
					'company_incor_date' => 'required|date|before:' . now()->format('Y-m-d'),
					'dob' => 'required|date|before_or_equal:' . now()->subYears(10)->format('Y-m-d'),
					...
					'email' => ['required'],
					'pan_card' => 'required',
					'registeredas' => 'required',
					'productCategory' => 'required',
					'businessRunBy' => 'required',
				...
				$pincode = $request->pincode;
				$loan_amount = $request->loan_amount;
				$registeredas = $request->registeredas;
				$gender = $request->gender;
				$address = $request->address;
				$pan_card = $request->pan_card;
				$dob = $request->dob;
				...
				$business_revenue = $request->business_revenue;
				$code = $request->access_code;
				$productCategory = $request->productCategory;
				$businessRunBy = $request->businessRunBy;
				...
				$loan->purpose_of_loan = $sub_service_id;
				$loan->gender = $gender;
				$loan->business_age = $business_age;
				$loan->business_revenue = $business_revenue;
				$loan->registeredas = $registeredas;
				$loan->pan_card = $pan_card;
				$loan->dob = $dob;
				$loan->company_incorporation_date = $company_incor_date;
				$loan->gst_no = $gst_no;
				...
				$prefix = "LoanSarovar_Partner-";
				$merged = $prefix . $loan->loan_id;
				// session()->forget('loan_signin_id');

				$loandetails = new Loandetail();
				$loandetails->loan_id = $loan->loan_id;
				$loandetails->unique_id = $merged;
				$loandetails->productCategory = $productCategory;
				$loandetails->businessRunBy = $businessRunBy;
				$loandetails->natureOfBusiness = $natureOfBusiness;
				$loandetails->business_address = $business_address;
				$loandetails->business_pincode = $business_pincode;
				$loandetails->save();
				...
				$headers = [
					'Content-Type' => 'application/json',
					'Accept' => 'application/json',
					'X-Api-Key' => 'f4e71480-f915-48b8-892c-5fafd033bfd0'
				];
				...
				$mailData = [
					...
					'body' => $loan->full_name,
					'type' => 'congrats',
					'content' => 'Your request for loan Application login successfully. thanks for connecting with Loan Sarovar. Kindly upload your documents',
				];
				Mail::to($loan->email)->send(new MailNotify($mailData));



				// $request->session()->put('success',"your application has been processed successfully. Kindly upload your documents");
				// if($response['message']=="ELIGIBLE"){
				if ($response->successful() && isset($responseData['message']) && $responseData['message'] === 'ELIGIBLE') {
					$bankresponse = new Bankresponse();
					$bankresponse->loan_id = $loan->loan_id;
					$bankresponse->application_id = $response['applicationId'];
					$bankresponse->message = $response['message'];
					$bankresponse->payload = $response['leadId'];
					$bankresponse->unique_id = $response['uniqueId'];
					$bankresponse->bank_id = $user_selected_bank;
					$bankresponse->save();

					$request->session()->put('success', "Congratulations! your application has been processed successfully. Kindly upload your documents.");
					return redirect('admin/document-upload/' . $loan->loan_id);
					...
					//$request->session()->put('error', $response['message']);
					$errorMessage = $responseData['message'] ?? $responseData['error'] ?? 'Application not eligible at this time.';
				...
				// 	return redirect('admin/services/form/apply-user');
				// }else{
				// 	return redirect('admin/direct-services?	='.$code);
				// }
			}
			if (isset($request['cancel'])) {
				$request->session()->put('cancel', "Customer Loan Cancelled Successfully!!");
			...
			if (isset($request['save'])) {
				$request->validate([
					...
					'pincode' => 'required|digits:6',
					'loan_amount' => 'required|numeric',
					'monthly_salary' => 'required|numeric|between:12000,999999',
					'gender' => 'required',
					...
					'state' => 'required',
					'city' => 'required',
					'pan_card' => 'required|regex:/[A-Z]{3}[P]{1}[A-Z]{1}[0-9]{4}[A-Z]{1}/',
					'dob' => 'required|date|before_or_equal:' . now()->subYears(10)->format('Y-m-d'),

				], [
					'user_name.regex' => 'The :attribute must be in the format of first name and last name.'
				]);
				$username = $request->user_name;
				$phone = $request->phone;
				$email = $request->email;
				...
				$nameParts = explode(" ", $loan->full_name);
				// dd($nameParts);die;
				// $customerdocs = new Custdocument();
				// $customerdocs->loan_id = $loan->loan_id;
				// $customerdocs->save();
				$firstName = $nameParts[0];
				...
				// dd($firstName, $lastName);die;
				$url = 'https://partners.cashe.co.in/partner/create_customer';


				$payload = [
					"partner_name" => 'LoanSarovar_Partner',
					"applicant_id" => 'LoanSarovar_Partner-' . $loan->loan_id,
					"loan_amount" => $loan->loan_amount,
					"Personal Information" => [
						"First Name" => $firstName,
						"Last Name" => $lastName,
				...
				$mailData = [
					...
					'body' => $loan->full_name,
					'type' => 'congrats',
					'content' => 'Your request for loan Application login successfully. thanks for connecting with Loan Sarovar.',
				...
				$bankresponse = new Bankresponse();
				$bankresponse->loan_id = $loan->loan_id;
				$bankresponse->status = $k['status'];
				$bankresponse->message = $k['message'];
				$bankresponse->check_sum = $signature;
				$bankresponse->payload = $k['payLoad'];
				$bankresponse->statuscode = $k['statusCode'];
				$bankresponse->save();
				if ($k['status'] == "OK") {
					$request->session()->put('success', "your application has been processed successfully. Kindly upload your documents");
					return redirect('admin/document-upload/' . $loan->loan_id);
				} else {
					$request->session()->put('error', $k['payLoad']);
					return redirect('admin/services/form/apply-user');
				}
			}
			if (isset($request['cancel'])) {
			...
			$request->session()->put('error', "Something Went Wrong Please try Again Later.");
			return redirect('admin/services/form/apply-user');
		}
	}
	...
	public function getdocuments(Request $request)
	{
		...
		//dd("Reached document upload");
		$loan_id = $request->loanid;
		$loan = Loan::where('loan_id', $loan_id)->first();

		$bankresponse = Bankresponse::where('message', "Successfully Uploaded")->where('loan_id', $loan_id)->get();

		$profile = Custdocument::where('loan_id', $loan_id)->where('filetype', "profile")->where('upload_status', 1)->first();
		$pancard = Custdocument::where('loan_id', $loan_id)->where('filetype', "pancard")->where('upload_status', 1)->first();
		$bankStatement = Custdocument::where('loan_id', $loan_id)->where('filetype', "bankStatement")->where('upload_status', 1)->first();

		$gstlending = Custdocument::where('loan_id', $loan_id)->where('filetype', "GSTCertification-RegProof")->where('upload_status', 1)->first();
		$pancardlending = Custdocument::where('loan_id', $loan_id)->where('filetype', "PanCard-Personal")->where('upload_status', 1)->first();
		$bankStatementlending = Custdocument::where('loan_id', $loan_id)->where('filetype', "BankStatement-Company")->where('upload_status', 1)->first();

		$custdocuments = Custdocument::where('loan_id', $loan_id)->get();
		// dd(Bankresponse::where('loan_id', 183)->first());

		// dd($filetypenew);
		return view('admin.document-upload')->with('loan', $loan)->with('gstlending', $gstlending)->with('pancardlending', $pancardlending)->with('bankStatementlending', $bankStatementlending)->with('custdocuments', $custdocuments)->with('profile', $profile)->with('pancard', $pancard)->with('bankStatement', $bankStatement);
	...
	public function postdocuments(Request $request)
	{
		...
		$loan_id = $request->loanid;
		$loan = Bankresponse::where('loan_id', $loan_id)->first();

		$bank = Loan::where('loan_id', $loan_id)->first();
		// dd($bank);
		$user_selected_bank = $bank->bank_service;
		if ($user_selected_bank == 53) {

			// dd($request->all());die;
			$apiKey = 'f4e71480-f915-48b8-892c-5fafd033bfd0';
			$applicationId = 'LAI-118879037';
			$documentType = 'BankStatement-Company';
			$bankName = 'SBIN';
			$bankAccountNo = '38423823781';
			$password = '12334';
			$file = $request->file('file');
			$fileType = $request->file_type;
			$url = "https://api.lendingkart.com/v2/partner/leads/documents?applicationId={$applicationId}&documentType={$documentType}";
			$fileContents = file_get_contents($file->getPathname());
			// dd($fileContents);
			//Commented for now to store the documents until we get valid api key for Lendingkart bank
			$client = new Client();

			$response = $client->post('https://api.lendingkart.com/v2/partner/leads/documents', [
				'multipart' => [
					[
						'name' => 'applicationId',
			...
			if ($status == 200) {
				...
				// dd($file);
				$custdocs = new Custdocument();
				$path = $file->store($fileType);
				$custdocs->loan_id = $loan->loan_id;
				$custdocs->filetype = $fileType;
				$custdocs->upload_status = 1;
				$custdocs->document = $path;
				$custdocs->save();

				$bankresponse = new Bankresponse();
				$bankresponse->loan_id = $loan->loan_id;
				$bankresponse->status = "OK";
				$bankresponse->message = "Successfully Uploaded";
				$bankresponse->filetype = $fileType;
				$bankresponse->bank_id = $user_selected_bank;
		...
		if ($user_selected_bank == 56) {
			$fileType = $request->file_type;
			$file = $request->file('file');
			// dd($loan->payload, "nisarg");

			// dd($file);
			$partnerName = 'LoanSarovar_Partner';
			$partnerCustomerId = $loan->payload;
			$secretKey = 'f2tRC~GU2Bv;C@sK"';
			$totalStrData = $partnerName . $partnerCustomerId . $fileType;

			$fileContents = file_get_contents($file->getPathname());
			// dd(file_get_contents($file->getPathname()));
			$allData = $totalStrData . $fileContents;
			$hmac = hash_hmac("sha1", $allData, $secretKey, true);
			$signature = base64_encode($hmac);
			// dd($signature);
			if ($fileType == 'profile') {
				// dd($file);
				$custdocs = new Custdocument();
				$path = $file->store('cust-profile');
				$custdocs->loan_id = $loan->loan_id;
				$custdocs->check_sum = $signature;
				$custdocs->filetype = $fileType;
			...
			if ($fileType == 'bankStatement') {
				// dd($file);
				$custdocs = new Custdocument();
				$path = $file->store('cust-bankStatement');
				$custdocs->loan_id = $loan->loan_id;
				$custdocs->check_sum = $signature;
				$custdocs->filetype = $fileType;
				$custdocs->document = $path;
			...
			$client = new Client();
			$response = $client->request('POST', 'https://partners.cashe.co.in/partner/document/upload', [
				'headers' => [
					'Check-Sum' => $signature,
				],
				'multipart' => [
					[
						'name' => 'partner_name',
						'contents' => $partnerName,
					],
					[
						'name' => 'partner_customer_id',
						'contents' => $partnerCustomerId,
					],
					[
						'name' => 'file_type',
						'contents' => $fileType,
			...
			$k = json_decode($response->getBody()->getContents(), true);
			// dd($k);
			$payload = $k['payLoad'];
			if (is_array($payload)) {
				$payload = implode(', ', array_values($payload));
			}
			$bankresponse = new Bankresponse();
			$bankresponse->loan_id = $loan->loan_id;
			$bankresponse->status = $k['status'];
			...
			$loan_signin_id = $request->session()->get('loan_signin_id');
			// dd($loan_signin_id);
			$findservice = Loan::where('loan_id', $loan->loan_id)->first();
			// dd($findservice);
			if ($k['status'] == "OK") {
				$statusCode = 200;
				$custdocs->upload_status = 1;
	...
	public function postdeletedocuments(Request $request)
	{
		$check_sum = $request->id;
		$find = Custdocument::where('check_sum', $check_sum)->first();
		$loan = Loan::where('loan_id', $find->loan_id)->first();
		$loan->status_id = null;
		$loan->save();
		// dd($check_sum, $request->id);die;
		$deletedoc = Custdocument::where('check_sum', $check_sum)->first();
		$deletedoc->delete();
		$bankres = Bankresponse::where('check_sum', $check_sum)->first();
		$bankres->delete();

		$isDeleted = true; // Replace with your deletion logic
...
// 		}
	...
	public function sendBulkWhatsapp()
	{
		$response = \Illuminate\Support\Facades\Http::withHeaders([
			'X-API-KEY' => env('BULK_SECRET_KEY'),
			// ])->post('https://loansarovar.com/api/send-bulk-whatsapp');
		])->post(env('WEBSITE_API_URL') . '/api/send-bulk-whatsapp');

		if ($response->successful()) {
			return back()->with('success', 'WhatsApp messages queued successfully!');
		}
